...
require File.join(ANSIBLE_PATH, 'lib', 'ftl')
...
ftl_config = FTL::Config.new(root_path: ANSIBLE_PATH)
...
Vagrant.configure('2') do |config|
  ...
  config.vm.hostname = ftl_config.site_host
  ...
  config.vm.provision provisioner do |ansible|
    ...
    if !Vagrant::Util::Platform.windows?
      config.trigger.after :up do |trigger|
        ...
        trigger.info = "Adding vagrant ssh-config for #{ftl_config.site_host} to ~/.ssh/config"
        trigger.ruby do
          update_ssh_config(ftl_config.site_host)
        end
      end
    end
  end
